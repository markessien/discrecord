...
Private Sub Form_Load()
    ...
    If CmbWriteSpeeds.ListCount > 0 Then
    ...
    FrmBurn.DiscRecord.QueryDriveInfo strName, strFirmware, nBufferSize, strDate, strSerial
    ...
    FrmBurn.DiscRecord.ListAvailableReadSpeeds

    If CmbReadSpeed.ListCount > 0 Then CmbReadSpeed.ListIndex = 0
    
    ChkCDR.Value = IIf(FrmBurn.DiscRecord.ReadFeatureSupported(featureCDR) = featureSupported, 1, 0)
    ChkCDRW.Value = IIf(FrmBurn.DiscRecord.ReadFeatureSupported(featureCDRW) = featureSupported, 1, 0)
    ChkDVDRAM.Value = IIf(FrmBurn.DiscRecord.ReadFeatureSupported(featureDVDRAM) = featureSupported, 1, 0)
    ChkDVDROM.Value = IIf(FrmBurn.DiscRecord.ReadFeatureSupported(featureDVDROM) = featureSupported, 1, 0)
    ChkDVDR.Value = IIf(FrmBurn.DiscRecord.ReadFeatureSupported(featureDVDR) = featureSupported, 1, 0)

    ChkCDR_w.Value = IIf(FrmBurn.DiscRecord.WriteFeatureSupported(featureCDR) = featureSupported, 1, 0)
    ChkCDRW_w.Value = IIf(FrmBurn.DiscRecord.WriteFeatureSupported(featureCDRW) = featureSupported, 1, 0)
    ChkDVDRAM_w.Value = IIf(FrmBurn.DiscRecord.WriteFeatureSupported(featureDVDRAM) = featureSupported, 1, 0)
    ChkBuf.Value = IIf(FrmBurn.DiscRecord.WriteFeatureSupported(featureBufferProtect) = featureSupported, 1, 0)
    ChkDVDR_w.Value = IIf(FrmBurn.DiscRecord.WriteFeatureSupported(featureDVDR) = featureSupported, 1, 0)
